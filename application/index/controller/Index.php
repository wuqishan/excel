...
class Index
{
    private $template = APP_PATH . '/data/template.xlsm';
    ...
    private function getForm($data)
    {
        foreach ($data as $k => $v) {
            ...
            if (isset($v['values']) && is_array($v['values']) && ! empty($v['values'])) {
                ...
                $data[$k]['input_str'] = '<div class="item_sel"><span>'.$v['title'].':</span>'.implode('', $temp).'</div>';
                $class[] = $k;
            }

            // 更新的内容为手写填充
            if (isset($v['values']) && $v['values'] === '') {
                $data[$k]['input_str'] = '<div class="item_sel"><span>'.$v['title'].':</span><input type="text" name="'.$k.'" value=""></div>';
                $class[] = $k;
            }
        }
